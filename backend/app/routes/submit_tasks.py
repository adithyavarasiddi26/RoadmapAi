...
import json
from .get_capstone import get_capstone_as_task
            ...
            if num_uncompleted_topics_in_phase == 0:
                ...
                if current_phase:
                    ...
                    if next_locked_phase:
                        ...
                        # No more phases to unlock, user has completed the roadmap
                        # Now enable capstone project
                        db.query(Current).filter(Current.user_id == current_user.id).update({
                            "current_phase_id": None,
                            "current_topic_id": None,
                            "current_topic": None,
                            "current_phase": None,
                            "capstone_active": True
                        })


                    db.commit()
                    # if capstone is active, then we don't need to generate daily tasks from llm since capstone tasks are fixed and not generated by llm. We can directly return from here.
                    if db.query(Current).filter(Current.user_id == current_user.id).first().capstone_active:
                        result = get_capstone_as_task(current_user.id)
                        return result
